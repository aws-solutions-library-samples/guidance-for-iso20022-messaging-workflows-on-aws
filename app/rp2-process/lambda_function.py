...
def lambda_handler(event, context):
    ...
    try:
        ...
        if 'Item' in response and response['Item']:
            metadata['ErrorMessage'] = 'received transaction is canceled'
            LOGGER.warning(f'{metadata["ErrorMessage"]}: {response}')
            item['transaction_code'] = 'TECH'
            response = dynamodb_put_item(region, table, item, replicated)
            LOGGER.debug(f'got response: {response}')
        ...
        LOGGER.debug(f'sent request: ACTC {item}')
        response = dynamodb_get_by_item(region, table, {**item, 'transaction_status': 'ACTC'})
        LOGGER.debug(f'got response: {response}')
        if 'Item' not in response or not response['Item']:
            metadata['ErrorMessage'] = 'received transaction is missing'
            LOGGER.warning(f'{metadata["ErrorMessage"]}: {response}')
